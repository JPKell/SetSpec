...
They do **not** define what a consumer installs. `pip install setspec` resolves the compatible
...
## The install mode these assume

CI installs the built distribution, not an editable checkout, so `pip install . --no-deps` follows
the lock in every job that needs the package. That is why `[tool.coverage.run] source` names the
**importable** package rather than `src/setspec`, with `[tool.coverage.paths]` folding the two
locations into one name: pointed at the source path, coverage measures a directory nothing imports
and reports 0 %, which fails the 95 % floor for a reason that has nothing to do with the tests.
